...
for t in topic:
  if t in ['de_purp','camp','pain']:
    ...
    temp = df[df['program'].isin(prgm) & df['length'].isin(lgth) & df['youth'].isin(youth) & df['district_vol'].isin(dist) & df['eagle'].isin(eagle)].groupby(by=t).count()['sub_date'].sort_values(ascending=True)
