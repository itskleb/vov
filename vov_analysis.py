...
df = pd.read_csv('Voice_of_the_Volunteer_20252025-02-19_10_00_11.csv')
...
for t in topic:
  if t in ['de_purp','camp','pain']:
    ...
    charty = px.bar(rankers(temp2,t),text_auto = True)
    charty.update_layout(showlegend = False)
    ...
    tot = write['sub_date'].sum(axis=1)
    write['percent'] = str(round(write['sub_date']/tot,2))+'%'
    st.write(write)
